...
<body>
    <?php
    // Replace the example $game initialization with:
    $game = @include('round-empty.php');
    if (!is_array($game) || !isset($game['categories'], $game['pointScale'])) {
        error_log("Game data file is invalid or missing required keys.", 0);
    ...
    <!-- Question form -->
    <?php if (isset($game['categories']) && !empty($game['categories'])): ?>
        ...
        <form method="POST" class="question-form">
            ...
            $category = $game['categories'][$categoryIndex];
            $questionIndex = array_rand($category['questions']);
            $questionData = $category['questions'][$questionIndex];

            // Ensure the actual question text is displayed
            ?>
            <p><strong>Category:</strong> <?php echo htmlentities($category['name']); ?></p>
    ...
    <?php else: ?>
        <p>No questions available. Please check the game configuration.</p>
    <?php endif; ?>
